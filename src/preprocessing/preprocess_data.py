...
def preprocessing(dataset, cf = cf):
    ...
    print(f"Training data shape: {X.shape}, Test data shape: {y.shape}, Groups shape: {groups.shape}")
...
def load_session_data(cf, session_id):
    ...
    """
    with open(cf.path_PPG_Dalia + f'PPG_FieldStudy/S{session_id}/S{session_id}.pkl', 'rb') as f:
...
def process_signals(session_data):
    ...
    """
    ppg = session_data['signal']['wrist']['BVP'][::2][38:]  # Downsample and trim PPG data
...
def create_windows(ppg, acc, activity, cf):
    ...
    window_shape = (cf.time_window * 32, 4)  # Example window: (time_window_samples, 4 features)
    ...
    # the length of each window dim to the last axis -> (4093 * 15 participants, 256 data points in 8 seconds, 4 columns)
...
def create_activity_windows(activity, cf):
    ...
    window_shape = (cf.time_window * 4, 1)  # 8 * 4 being 32 mean the window size is (32, 1)
